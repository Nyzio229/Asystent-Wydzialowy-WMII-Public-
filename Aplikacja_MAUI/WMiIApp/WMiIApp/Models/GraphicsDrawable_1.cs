...
namespace WMiIApp.Models
{
    public class GraphicsDrawable_1 : IDrawable
    {
        public void Draw(ICanvas canvas, RectF dirtyRect)
        {
            ...
            {
                // Sprawdzamy, czy pokoj nalezy do poziomu -1
                if (room.Floor == -1)
                {
                    // Dodajemy pare (x, y) do listy punktow
                    points.Add(new Tuple<float, float, string>((float)room.PositionX, (float)room.PositionY, room.Name));
                }
            }
            ...
            for (int i = 0; i < points.Count - 1; i++)
            {
                ...
                if (!(startPoint.Item3 == "Schody do Piwnicy (Piwnica)" && endPoint.Item3 == "Schody Piwnica/Parter/I piętro (Piwnica)") && !(startPoint.Item3 == "Schody Piwnica/Parter/I piętro (Piwnica)" && endPoint.Item3 == "Schody do Piwnicy (Piwnica)"))
                {
